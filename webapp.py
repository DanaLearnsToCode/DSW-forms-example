    ...
    if answer1 == 'yes':
        ...
        return render_template('response2.html', response = reply)
            if answer2 == "horror"
                ...
                return render_template('response3.1.html', response = reply)
            if answer2 == "comedy"
                reply = "I also prefer comedy because I like to laugh, but horror movies are still fun to watch"
                return render_template('response3.2.html', response = reply)
            if answer2 == "yes"
                reply = "I'm also a sports person."
                return render_template('response3.3.html', response = reply)
            if answer2 == "no"
                reply = "Wow! most people I know play sports, so this is very cool."
                return render_template('response3.4.html', response = reply)
